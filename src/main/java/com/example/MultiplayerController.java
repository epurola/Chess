...
public class MultiplayerController {
        ...
        if (!isServerRunning) {
            Task<Void> serverTask = new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    ...
                    webSocketServer = ChessWebSocketServer.getInstance(new InetSocketAddress(localIPAddress, 8887));

                    webSocketServer.start();
                    isServerRunning = true;
                    Platform.runLater(() -> {
                        serverIpField.setText(localIPAddress); // Display the IP address in serverIpField
                    });
                    return null;
                }
            };
    ...
    private void handlePieceDrag(MouseEvent event) {
        ...
        if (draggedPiece != null) {
            draggedPiece.setTranslateX(
                    event.getSceneX() - 50 - draggedPiece.getLayoutX() - chessBoard.localToScene(0, 0).getX());
            draggedPiece.setTranslateY(
    ...
    private void drawPossibleMoves(Piece selectedPiece) {
        ...
        for (int[] move : possibleMoves) {
            ...
            if (isOccupied) {
                ...
                moveIndicator.setStroke(moveHelpColor.deriveColor(0, 1, 1, 0.5)); // Set stroke color with transparency
                                                                                  // (50% opacity)
                moveIndicator.setStrokeWidth(4); // Adjust the stroke width if needed
                moveIndicator.setRadius(40);
            } else {
                moveIndicator.setFill(moveHelpColor.deriveColor(0, 1, 1, 0.5)); // Set fill color with transparency (50%
                                                                                // opacity)
            }
    ...
    public void updateGameState(String pieceName, int fromRow, int fromCol, int movedRow, int movedCol,
